// src/i18n/ui.ts

export type Lang = "zh-CN" | "zh-TW" | "en" | "es" | "pt";

export const DEFAULT_LANG: Lang = "zh-CN";

export const LANGUAGES: Record<Lang, string> = {
  "zh-CN": "简体中文",
  "zh-TW": "繁體中文",
  "en":    "English",
  "es":    "Español",
  "pt":    "Português",
};

export const HTML_LANG: Record<Lang, string> = {
  "zh-CN": "zh-CN",
  "zh-TW": "zh-TW",
  "en":    "en",
  "es":    "es",
  "pt":    "pt-BR",
};

// URL 路径段（空字符串 = 默认语言/根路径）
export const LANG_TO_PATH: Record<Lang, string> = {
  "zh-CN": "",
  "zh-TW": "zh-tw",
  "en":    "en",
  "es":    "es",
  "pt":    "pt",
};

// URL 路径段（小写） → 内部 Lang key
export const LANG_PATH_MAP: Record<string, Lang> = {
  "zh-tw": "zh-TW",
  "en":    "en",
  "es":    "es",
  "pt":    "pt",
};

// 浏览器语言 → Lang key
export const BROWSER_LANG_MAP: Record<string, Lang> = {
  "zh":    "zh-CN",
  "zh-CN": "zh-CN",
  "zh-TW": "zh-TW",
  "zh-HK": "zh-TW",
  "zh-MO": "zh-TW",
  "en":    "en",
  "en-US": "en",
  "en-GB": "en",
  "es":    "es",
  "es-ES": "es",
  "es-MX": "es",
  "pt":    "pt",
  "pt-BR": "pt",
  "pt-PT": "pt",
};

// 所有非默认语言的路径段列表
export const LANG_PATH_SEGMENTS: string[] = ["zh-tw", "en", "es", "pt"];

// ── 左侧导航分类结构 ──────────────────────────────────────
export const TAXONOMY = [
  {
    key: "home",
    href: "/",
    sub: [] as { key: string; labelKey: string; href: string }[],
  },
  {
    key: "news",
    href: "/posts",
    sub: [] as { key: string; labelKey: string; href: string }[],
  },
  {
    key: "prices",
    href: "/price",
    sub: [
      { key: "BTC",  labelKey: "cat.btc",    href: "/coin/btc" },
      { key: "ETH",  labelKey: "cat.eth",    href: "/coin/eth" },
      { key: "行情", labelKey: "cat.market", href: "/price" },
    ],
  },
  {
    key: "topics",
    href: "/news",
    sub: [
      { key: "news",       labelKey: "cat.news",        href: "/news" },
      { key: "bitcoin",    labelKey: "cat.bitcoin",     href: "/bitcoin" },
      { key: "coins",      labelKey: "cat.coinswiki",   href: "/coins" },
      { key: "blockchain", labelKey: "cat.blockchain",  href: "/blockchain" },
      { key: "wallet",     labelKey: "cat.wallet",      href: "/wallet" },
      { key: "exchange",   labelKey: "cat.exchange",    href: "/exchange" },
      { key: "basics",     labelKey: "cat.basics",      href: "/basics" },
    ],
  },
  {
    key: "tools",
    href: "/tools/fee-calculator",
    sub: [
      { key: "手续费计算", labelKey: "nav.tools.fee",       href: "/tools/fee-calculator" },
      { key: "投资组合",   labelKey: "nav.tools.portfolio",  href: "/portfolio" },
      { key: "恐惧贪婪",   labelKey: "nav.tools.feargreed", href: "/fear-greed" },
      { key: "Gas追踪",   labelKey: "nav.tools.gas",       href: "/tools/gas" },
      { key: "DeFi TVL",  labelKey: "nav.tools.defi",      href: "/tools/defi" },
      { key: "术语词典",   labelKey: "nav.tools.glossary",  href: "/glossary" },
      { key: "巨鲸追踪",   labelKey: "nav.tools.whale",    href: "/tools/whale" },
    ],
  },
  {
    key: "defi",
    href: "/tags/defi",
    sub: [
      { key: "DeFi", labelKey: "cat.defi", href: "/tags/defi" },
      { key: "NFT",  labelKey: "cat.nft",  href: "/tags/nft" },
    ],
  },
];

// ── 翻译字典 ──────────────────────────────────────────────
export const UI: Record<Lang, Record<string, string>> = {
  "zh-CN": {
    "site.name":        "Bitaigen",
    "site.desc":        "Bitaigen（比特根）是专业加密货币与区块链资讯平台，提供比特币BTC实时行情、币安Binance/OKX注册教程、DeFi入门、加密钱包安全指南，助力新手快速入门加密投资。",
    "site.desc.short":  "Bitaigen（比特根）— 专业加密货币与区块链资讯",
    "home.h1":          "Bitaigen（比特根）— 比特币行情、加密货币新闻与交易所教程",
    "home.title":       "Bitaigen（比特根）— 比特币行情、加密货币资讯与交易所教程",
    "site.keywords":    "比特根,比特币,加密货币,区块链,BTC价格,币安教程,OKX注册,DeFi,NFT,钱包安全,以太坊,加密投资",
    "nav.home":         "首页",
    "nav.flash":        "快讯",
    "nav.news":         "资讯",
    "nav.newsCenter":   "资讯中心",
    "nav.prices":       "行情",
    "nav.learn":        "学习中心",
    "nav.topics":       "话题分类",
    "nav.search":       "搜索",
    "nav.about":        "关于",
    "section.latest":   "最新资讯",
    "section.all":      "全部文章",
    "section.more":     "查看更多",
    "section.flash":    "快讯",
    "section.hot":      "热门文章",
    "section.tutorial": "教程指南",
    "section.related":  "相关文章",
    "section.tags":     "热门标签",
    "section.topic":    "热门话题",
    "section.about":    "关于本站",
    // ── Topbar ──
    "topbar.prices":    "实时行情",
    "topbar.binance":   "币安教程",
    "topbar.telegram":  "Telegram",
    "topbar.follow":    "关注我们",
    "topbar.search":    "搜索",
    "topbar.subscribe": "订阅",
    "topbar.login":     "联系我们",
    // ── Footer ──
    "footer.col.news":  "加密资讯",
    "footer.col.guide": "新手指南",
    "footer.col.links": "快速链接",
    "footer.col.social":"社交媒体",
    "footer.about":     "关于我们",
    "footer.rights":    "保留所有权利",
    "footer.slogan":    "专业区块链资讯，助力投资决策",
    "footer.disclaimer":"免责声明：本站内容仅供参考，不构成投资建议。加密货币投资风险较高，请审慎决策。",
    // ── Post ──
    "post.prev":        "上一篇",
    "post.next":        "下一篇",
    "post.toc":         "目录",
    "post.author":      "作者",
    "post.translations":"其他语言版本",
    "post.readtime":    "阅读时间",
    "post.share":       "分享文章",
    "pagination.prev":  "上一页",
    "pagination.next":  "下一页",
    // ── About sections ──
    "about.btc.title":  "比特币资讯",
    "about.btc.desc":   "提供比特币实时行情、价格分析与深度报道，帮助您了解最新市场动态。",
    "about.ex.title":   "交易所教程",
    "about.ex.desc":    "币安、OKX 等主流交易所完整使用教程，从注册到交易一步步教会您。",
    "about.wallet.title":"钱包安全",
    "about.wallet.desc":"MetaMask、Ledger 等钱包使用指南，保护您的数字资产安全。",
    // ── Search page ──
    "search.title":     "搜索",
    "search.subtitle":  "搜索文章、教程、行情分析…",
    "search.hot":       "热门搜索",
    "search.quicknav":  "快速导航",
    "search.btc":       "BTC行情",
    "search.eth":       "ETH行情",
    "search.exchange":  "币安教程",
    "search.beginner":  "新手入门",
    "search.wallet":    "钱包安全",
    "search.defi":      "DeFi资讯",
    // ── Archives ──
    "archives.title":   "Ghost 文章归档",
    "archives.articles":"篇文章",
    "archives.unavailable":"内容暂时无法加载",
    "archives.unavailableDesc":"Ghost CMS 服务暂时不可用，请稍后刷新重试。",
    "archives.empty":   "暂无文章",
    "archives.local":   "本地文章列表",
    // ── Actions ──
    "btn.submit":       "投稿报道",
    "btn.partner":      "商务合作",
    "btn.telegram":     "加入 Telegram",
    // ── Lang ──
    "lang.label":       "语言",
    "lang.auto":        "自动检测",
    // ── Categories ──
    "cat.btc":          "BTC",
    "cat.eth":          "ETH",
    "cat.market":       "行情",
    "cat.beginner":     "新手入门",
    "cat.news":         "资讯",
    "cat.bitcoin":      "比特币",
    "cat.coinswiki":    "币种百科",
    "cat.blockchain":   "区块链技术",
    "cat.basics":       "币圈科普",
    "cat.exchange":     "交易所",
    "cat.wallet":       "钱包",
    "cat.defi":         "DeFi",
    "cat.nft":          "NFT",
    "cat.tutorial":     "教程",
    "cat.market_key":   "market",
    "cat.tutorial_key": "tutorial",
    "cat.defi_key":     "defi",
    // ── Footer links ──
    "footer.link.btc":       "BTC 行情",
    "footer.link.eth":       "ETH 资讯",
    "footer.link.defi":      "DeFi 动态",
    "footer.link.nft":       "NFT 资讯",
    "footer.link.market":    "市场分析",
    "footer.link.beginner":  "新手入门",
    "footer.link.exchange":  "交易所教程",
    "footer.link.wallet":    "钱包安全",
    "footer.link.prices":    "实时行情",
    "footer.link.btc_price": "BTC 价格",
    "footer.link.all":       "全部文章",
    "footer.link.tags":      "标签索引",
    "footer.link.archives":  "归档",
    // ── Subscribe ──
    "subscribe.title":       "订阅 Bitaigen 快报",
    "subscribe.desc":        "每周精选加密货币资讯、BTC 行情分析直达您的邮箱",
    "subscribe.placeholder": "您的邮箱地址",
    "subscribe.btn":         "免费订阅",
    "subscribe.success":     "✓ 订阅成功！请查收欢迎邮件",
    "subscribe.pending":     "✓ 请查收邮箱并点击确认链接以完成订阅",
    "subscribe.error":       "订阅失败，请稍后重试",
    "subscribe.rate":        "请求过于频繁，请稍后再试",
    "subscribe.invalid":     "请输入有效的邮箱地址",
    "subscribe.privacy":     "我们重视您的隐私，绝不发送垃圾邮件",
    // ── Cookie consent ──
    "cookie.message":        "本站使用 Cookie 和分析工具以提升浏览体验。",
    "cookie.accept":         "接受",
    "cookie.decline":        "仅必要",
    "cookie.learnMore":      "隐私政策",
    // ── Retention features ──
    "nav.tools":             "工具",
    "nav.tools.fee":         "手续费计算器",
    "nav.tools.portfolio":   "投资组合",
    "nav.tools.feargreed":   "恐惧贪婪指数",
    "nav.tools.gas":         "Gas追踪器",
    "nav.tools.defi":        "DeFi TVL",
    "nav.tools.glossary":    "术语词典",
    "nav.tools.whale":       "巨鲸追踪",
    "bnav.home":             "首页",
    "bnav.flash":            "快讯",
    "bnav.prices":           "行情",
    "bnav.watchlist":        "自选",
    "bnav.search":           "搜索",
    "watchlist.title":       "我的自选",
    "watchlist.empty":       "点击 ☆ 添加关注币种",
    "watchlist.since":       "自上次访问",
    "history.title":         "最近阅读",
    "history.empty":         "暂无阅读记录",
    "history.clear":         "清空",
  },
  "zh-TW": {
    "site.name":        "Bitaigen",
    "site.desc":        "Bitaigen（比特根）是專業加密貨幣與區塊鏈資訊平台，提供比特幣BTC即時行情、幣安Binance/OKX注冊教學、DeFi入門、加密錢包安全指南，助力新手快速入門加密投資。",
    "site.desc.short":  "Bitaigen（比特根）— 專業加密貨幣與區塊鏈資訊",
    "home.h1":          "Bitaigen（比特根）— 比特幣行情、加密貨幣新聞與交易所教學",
    "home.title":       "Bitaigen（比特根）— 比特幣行情、加密貨幣資訊與交易所教學",
    "site.keywords":    "比特根,比特幣,加密貨幣,區塊鏈,BTC價格,幣安教學,OKX注冊,DeFi,NFT,錢包安全,以太坊,加密投資",
    "nav.home":         "首頁",
    "nav.flash":        "快訊",
    "nav.news":         "資訊",
    "nav.newsCenter":   "資訊中心",
    "nav.prices":       "行情",
    "nav.learn":        "學習中心",
    "nav.topics":       "話題分類",
    "nav.search":       "搜尋",
    "nav.about":        "關於",
    "section.latest":   "最新資訊",
    "section.all":      "全部文章",
    "section.more":     "查看更多",
    "section.flash":    "快訊",
    "section.hot":      "熱門文章",
    "section.tutorial": "教學指南",
    "section.related":  "相關文章",
    "section.tags":     "熱門標籤",
    "section.topic":    "熱門話題",
    "section.about":    "關於本站",
    "topbar.prices":    "即時行情",
    "topbar.binance":   "幣安教學",
    "topbar.telegram":  "Telegram",
    "topbar.follow":    "關注我們",
    "topbar.search":    "搜尋",
    "topbar.subscribe": "訂閱",
    "topbar.login":     "聯繫我們",
    "footer.col.news":  "加密資訊",
    "footer.col.guide": "新手指南",
    "footer.col.links": "快速連結",
    "footer.col.social":"社交媒體",
    "footer.about":     "關於我們",
    "footer.rights":    "保留所有權利",
    "footer.slogan":    "專業區塊鏈資訊，助力投資決策",
    "footer.disclaimer":"免責聲明：本站內容僅供參考，不構成投資建議。加密貨幣投資風險較高，請審慎決策。",
    "post.prev":        "上一篇",
    "post.next":        "下一篇",
    "post.toc":         "目錄",
    "post.author":      "作者",
    "post.translations":"其他語言版本",
    "post.readtime":    "閱讀時間",
    "post.share":       "分享文章",
    "pagination.prev":  "上一頁",
    "pagination.next":  "下一頁",
    "search.title":     "搜尋",
    "search.subtitle":  "搜尋文章、教學、行情分析…",
    "search.hot":       "熱門搜尋",
    "search.quicknav":  "快速導航",
    "search.btc":       "BTC行情",
    "search.eth":       "ETH行情",
    "search.exchange":  "交易所教學",
    "search.beginner":  "新手入門",
    "search.wallet":    "錢包安全",
    "search.defi":      "DeFi資訊",
    "archives.title":   "Ghost 文章歸檔",
    "archives.articles":"篇文章",
    "archives.unavailable":"內容暫時無法載入",
    "archives.unavailableDesc":"Ghost CMS 服務暫時不可用，請稍後刷新重試。",
    "archives.empty":   "暫無文章",
    "archives.local":   "本地文章列表",
    "about.btc.title":  "比特幣資訊",
    "about.btc.desc":   "提供比特幣即時行情、價格分析與深度報道，幫助您了解最新市場動態。",
    "about.ex.title":   "交易所教學",
    "about.ex.desc":    "幣安、OKX 等主流交易所完整使用教學，從註冊到交易一步步教會您。",
    "about.wallet.title":"錢包安全",
    "about.wallet.desc":"MetaMask、Ledger 等錢包使用指南，保護您的數位資產安全。",
    "btn.submit":       "投稿報道",
    "btn.partner":      "商務合作",
    "btn.telegram":     "加入 Telegram",
    "lang.label":       "語言",
    "lang.auto":        "自動偵測",
    "cat.btc":          "BTC",
    "cat.eth":          "ETH",
    "cat.market":       "行情",
    "cat.beginner":     "新手入門",
    "cat.news":         "資訊",
    "cat.bitcoin":      "比特幣",
    "cat.coinswiki":    "幣種百科",
    "cat.blockchain":   "區塊鏈技術",
    "cat.basics":       "幣圈科普",
    "cat.exchange":     "交易所",
    "cat.wallet":       "錢包",
    "cat.defi":         "DeFi",
    "cat.nft":          "NFT",
    "cat.tutorial":     "教學",
    // ── Footer links ──
    "footer.link.btc":       "BTC 行情",
    "footer.link.eth":       "ETH 資訊",
    "footer.link.defi":      "DeFi 動態",
    "footer.link.nft":       "NFT 資訊",
    "footer.link.market":    "市場分析",
    "footer.link.beginner":  "新手入門",
    "footer.link.exchange":  "交易所教學",
    "footer.link.wallet":    "錢包安全",
    "footer.link.prices":    "即時行情",
    "footer.link.btc_price": "BTC 價格",
    "footer.link.all":       "全部文章",
    "footer.link.tags":      "標籤索引",
    "footer.link.archives":  "歸檔",
    // ── Subscribe ──
    "subscribe.title":       "訂閱 Bitaigen 快報",
    "subscribe.desc":        "每週精選加密貨幣資訊、BTC 行情分析直達您的郵箱",
    "subscribe.placeholder": "您的電子郵件地址",
    "subscribe.btn":         "免費訂閱",
    "subscribe.success":     "✓ 訂閱成功！請查收歡迎郵件",
    "subscribe.pending":     "✓ 請查收郵箱並點擊確認連結以完成訂閱",
    "subscribe.error":       "訂閱失敗，請稍後重試",
    "subscribe.rate":        "請求過於頻繁，請稍後再試",
    "subscribe.invalid":     "請輸入有效的電子郵件地址",
    "subscribe.privacy":     "我們重視您的隱私，絕不發送垃圾郵件",
    "cookie.message":        "本站使用 Cookie 和分析工具以提升瀏覽體驗。",
    "cookie.accept":         "接受",
    "cookie.decline":        "僅必要",
    "cookie.learnMore":      "隱私政策",
    "nav.tools":             "工具",
    "nav.tools.fee":         "手續費計算器",
    "nav.tools.portfolio":   "投資組合",
    "nav.tools.feargreed":   "恐懼貪婪指數",
    "bnav.home":             "首頁",
    "bnav.flash":            "快訊",
    "bnav.prices":           "行情",
    "bnav.watchlist":        "自選",
    "bnav.search":           "搜尋",
    "watchlist.title":       "我的自選",
    "watchlist.empty":       "點擊 ☆ 添加關注幣種",
    "watchlist.since":       "自上次訪問",
    "history.title":         "最近閱讀",
    "history.empty":         "暫無閱讀記錄",
    "history.clear":         "清空",
  },
  "en": {
    "site.name":        "Bitaigen",
    "site.desc":        "Bitaigen — Bitcoin price, crypto news & exchange tutorials. Binance/OKX setup guides, DeFi explained, wallet security and blockchain analysis for beginners and traders.",
    "site.desc.short":  "Bitaigen — Bitcoin price, crypto news & exchange guides",
    "home.h1":          "Bitaigen — Bitcoin Price, Crypto News & Exchange Tutorials",
    "home.title":       "Bitaigen — Bitcoin Price, Crypto News & Exchange Tutorials",
    "site.keywords":    "Bitcoin price,cryptocurrency news,Binance tutorial,OKX guide,DeFi,crypto wallet,blockchain,Ethereum,crypto for beginners,BTC",
    "nav.home":         "Home",
    "nav.flash":        "Flash",
    "nav.news":         "News",
    "nav.newsCenter":   "News Center",
    "nav.prices":       "Market",
    "nav.learn":        "Learn",
    "nav.topics":       "Topics",
    "nav.search":       "Search",
    "nav.about":        "About",
    "section.latest":   "Latest News",
    "section.all":      "All Articles",
    "section.more":     "View More",
    "section.flash":    "Flash News",
    "section.hot":      "Trending",
    "section.tutorial": "Tutorials",
    "section.related":  "Related Articles",
    "section.tags":     "Popular Tags",
    "section.topic":    "Topics",
    "section.about":    "About Us",
    "topbar.prices":    "Live Prices",
    "topbar.binance":   "Binance Guide",
    "topbar.telegram":  "Telegram",
    "topbar.follow":    "Follow Us",
    "topbar.search":    "Search",
    "topbar.subscribe": "Subscribe",
    "topbar.login":     "Contact",
    "footer.col.news":  "Crypto News",
    "footer.col.guide": "Beginner Guide",
    "footer.col.links": "Quick Links",
    "footer.col.social":"Social Media",
    "footer.about":     "About Us",
    "footer.rights":    "All rights reserved",
    "footer.slogan":    "Professional Blockchain Intelligence",
    "footer.disclaimer":"Disclaimer: Content on this site is for informational purposes only and does not constitute investment advice. Crypto investments carry significant risk.",
    "post.prev":        "Previous",
    "post.next":        "Next",
    "post.toc":         "Table of Contents",
    "post.author":      "Author",
    "post.translations":"Other languages",
    "post.readtime":    "Read time",
    "post.share":       "Share",
    "pagination.prev":  "Prev",
    "pagination.next":  "Next",
    "search.title":     "Search",
    "search.subtitle":  "Search articles, tutorials, market analysis…",
    "search.hot":       "Popular Searches",
    "search.quicknav":  "Quick Navigation",
    "search.btc":       "BTC Price",
    "search.eth":       "ETH Price",
    "search.exchange":  "Exchange Guides",
    "search.beginner":  "Beginner Guide",
    "search.wallet":    "Wallet Security",
    "search.defi":      "DeFi News",
    "archives.title":   "Ghost Archives",
    "archives.articles":"articles",
    "archives.unavailable":"Content temporarily unavailable",
    "archives.unavailableDesc":"Ghost CMS is temporarily unavailable. Please try again later.",
    "archives.empty":   "No articles found",
    "archives.local":   "Local articles",
    "about.btc.title":  "Bitcoin News",
    "about.btc.desc":   "Real-time Bitcoin price, market analysis and in-depth reporting to keep you informed.",
    "about.ex.title":   "Exchange Guides",
    "about.ex.desc":    "Complete tutorials for Binance, OKX and other major exchanges, from sign-up to trading.",
    "about.wallet.title":"Wallet Security",
    "about.wallet.desc":"MetaMask, Ledger and other wallet guides to keep your digital assets safe.",
    "btn.submit":       "Submit Story",
    "btn.partner":      "Partner With Us",
    "btn.telegram":     "Join Telegram",
    "lang.label":       "Language",
    "lang.auto":        "Auto detect",
    "cat.btc":          "BTC",
    "cat.eth":          "ETH",
    "cat.market":       "Market",
    "cat.beginner":     "Beginner",
    "cat.news":         "News",
    "cat.bitcoin":      "Bitcoin",
    "cat.coinswiki":    "Coin Wiki",
    "cat.blockchain":   "Blockchain",
    "cat.basics":       "Crypto Basics",
    "cat.exchange":     "Exchange",
    "cat.wallet":       "Wallet",
    "cat.defi":         "DeFi",
    "cat.nft":          "NFT",
    "cat.tutorial":     "Tutorial",
    // ── Footer links ──
    "footer.link.btc":       "BTC News",
    "footer.link.eth":       "ETH News",
    "footer.link.defi":      "DeFi Updates",
    "footer.link.nft":       "NFT News",
    "footer.link.market":    "Market Analysis",
    "footer.link.beginner":  "Beginner Guide",
    "footer.link.exchange":  "Exchange Tutorials",
    "footer.link.wallet":    "Wallet Security",
    "footer.link.prices":    "Live Prices",
    "footer.link.btc_price": "BTC Price",
    "footer.link.all":       "All Articles",
    "footer.link.tags":      "Tag Index",
    "footer.link.archives":  "Archives",
    // ── Subscribe ──
    "subscribe.title":       "Subscribe to Bitaigen",
    "subscribe.desc":        "Weekly crypto news, Bitcoin price analysis delivered to your inbox",
    "subscribe.placeholder": "Your email address",
    "subscribe.btn":         "Subscribe Free",
    "subscribe.success":     "✓ Subscribed! Check your inbox for a welcome email",
    "subscribe.pending":     "✓ Check your inbox and click the confirmation link",
    "subscribe.error":       "Subscription failed, please try again",
    "subscribe.rate":        "Too many attempts — please try again later",
    "subscribe.invalid":     "Please enter a valid email address",
    "subscribe.privacy":     "We respect your privacy. No spam, ever.",
    "cookie.message":        "We use cookies and analytics to improve your experience.",
    "cookie.accept":         "Accept",
    "cookie.decline":        "Essential only",
    "cookie.learnMore":      "Privacy Policy",
    "nav.tools":             "Tools",
    "nav.tools.fee":         "Fee Calculator",
    "nav.tools.portfolio":   "Portfolio",
    "nav.tools.feargreed":   "Fear & Greed",
    "nav.tools.gas":         "Gas Tracker",
    "nav.tools.defi":        "DeFi TVL",
    "nav.tools.glossary":    "Glossary",
    "nav.tools.whale":       "Whale Tracker",
    "bnav.home":             "Home",
    "bnav.flash":            "Flash",
    "bnav.prices":           "Prices",
    "bnav.watchlist":        "Watchlist",
    "bnav.search":           "Search",
    "watchlist.title":       "My Watchlist",
    "watchlist.empty":       "Tap ☆ to add coins",
    "watchlist.since":       "since last visit",
    "history.title":         "Recently Read",
    "history.empty":         "No reading history yet",
    "history.clear":         "Clear",
  },
  "es": {
    "site.name":        "Bitaigen",
    "site.desc":        "Bitaigen — precio de Bitcoin, noticias cripto y tutoriales de exchanges. Guías Binance/OKX, introducción a DeFi, seguridad de billeteras y análisis blockchain para principiantes.",
    "site.desc.short":  "Bitaigen — precio Bitcoin, noticias cripto y guías de exchange",
    "home.h1":          "Bitaigen — Precio de Bitcoin, Noticias Cripto y Tutoriales de Exchanges",
    "home.title":       "Bitaigen — Precio de Bitcoin, Noticias Cripto y Tutoriales",
    "site.keywords":    "precio Bitcoin,criptomonedas,tutorial Binance,guía OKX,DeFi,billetera cripto,blockchain,Ethereum,principiantes cripto,BTC",
    "nav.home":         "Inicio",
    "nav.flash":        "Flash",
    "nav.news":         "Noticias",
    "nav.newsCenter":   "Centro de Noticias",
    "nav.prices":       "Mercado",
    "nav.learn":        "Aprender",
    "nav.topics":       "Temas",
    "nav.search":       "Buscar",
    "nav.about":        "Acerca de",
    "section.latest":   "Últimas Noticias",
    "section.all":      "Todos los Artículos",
    "section.more":     "Ver Más",
    "section.flash":    "Noticias Rápidas",
    "section.hot":      "Tendencias",
    "section.tutorial": "Tutoriales",
    "section.related":  "Artículos Relacionados",
    "section.tags":     "Etiquetas Populares",
    "section.topic":    "Temas",
    "section.about":    "Sobre Nosotros",
    "topbar.prices":    "Precios en Vivo",
    "topbar.binance":   "Guía Binance",
    "topbar.telegram":  "Telegram",
    "topbar.follow":    "Síguenos",
    "topbar.search":    "Buscar",
    "topbar.subscribe": "Suscribirse",
    "topbar.login":     "Contacto",
    "footer.col.news":  "Noticias Cripto",
    "footer.col.guide": "Guía para Principiantes",
    "footer.col.links": "Enlaces Rápidos",
    "footer.col.social":"Redes Sociales",
    "footer.about":     "Sobre Nosotros",
    "footer.rights":    "Todos los derechos reservados",
    "footer.slogan":    "Inteligencia Profesional de Blockchain",
    "footer.disclaimer":"Aviso: El contenido de este sitio es solo informativo y no constituye asesoramiento de inversión.",
    "post.prev":        "Anterior",
    "post.next":        "Siguiente",
    "post.toc":         "Índice",
    "post.author":      "Autor",
    "post.translations":"Otros idiomas",
    "post.readtime":    "Tiempo de lectura",
    "post.share":       "Compartir",
    "pagination.prev":  "Anterior",
    "pagination.next":  "Siguiente",
    "search.title":     "Buscar",
    "search.subtitle":  "Buscar artículos, tutoriales, análisis de mercado…",
    "search.hot":       "Búsquedas Populares",
    "search.quicknav":  "Navegación Rápida",
    "search.btc":       "Precio BTC",
    "search.eth":       "Precio ETH",
    "search.exchange":  "Guías de Exchange",
    "search.beginner":  "Guía Principiante",
    "search.wallet":    "Seguridad Billetera",
    "search.defi":      "Noticias DeFi",
    "archives.title":   "Archivo Ghost",
    "archives.articles":"artículos",
    "archives.unavailable":"Contenido no disponible temporalmente",
    "archives.unavailableDesc":"El servicio Ghost CMS no está disponible. Inténtelo más tarde.",
    "archives.empty":   "No se encontraron artículos",
    "archives.local":   "Artículos locales",
    "about.btc.title":  "Noticias Bitcoin",
    "about.btc.desc":   "Precio de Bitcoin en tiempo real, análisis de mercado e informes detallados.",
    "about.ex.title":   "Guías de Exchange",
    "about.ex.desc":    "Tutoriales completos de Binance, OKX y otros exchanges principales.",
    "about.wallet.title":"Seguridad de Billetera",
    "about.wallet.desc":"Guías de MetaMask, Ledger y otras billeteras para proteger tus activos.",
    "btn.submit":       "Enviar Noticia",
    "btn.partner":      "Ser Socio",
    "btn.telegram":     "Unirse a Telegram",
    "lang.label":       "Idioma",
    "lang.auto":        "Detección automática",
    "cat.btc":          "BTC",
    "cat.eth":          "ETH",
    "cat.market":       "Mercado",
    "cat.beginner":     "Principiante",
    "cat.news":         "Noticias",
    "cat.bitcoin":      "Bitcoin",
    "cat.coinswiki":    "Enciclopedia de Monedas",
    "cat.blockchain":   "Blockchain",
    "cat.basics":       "Básicos de Cripto",
    "cat.exchange":     "Exchange",
    "cat.wallet":       "Billetera",
    "cat.defi":         "DeFi",
    "cat.nft":          "NFT",
    "cat.tutorial":     "Tutorial",
    // ── Footer links ──
    "footer.link.btc":       "BTC Noticias",
    "footer.link.eth":       "ETH Noticias",
    "footer.link.defi":      "DeFi Actualizaciones",
    "footer.link.nft":       "NFT Noticias",
    "footer.link.market":    "Análisis de Mercado",
    "footer.link.beginner":  "Guía Principiante",
    "footer.link.exchange":  "Tutoriales Exchange",
    "footer.link.wallet":    "Seguridad Billetera",
    "footer.link.prices":    "Precios en Vivo",
    "footer.link.btc_price": "Precio BTC",
    "footer.link.all":       "Todos los Artículos",
    "footer.link.tags":      "Índice de Etiquetas",
    "footer.link.archives":  "Archivo",
    // ── Subscribe ──
    "subscribe.title":       "Suscríbete a Bitaigen",
    "subscribe.desc":        "Noticias cripto semanales y análisis de precio de Bitcoin en tu bandeja",
    "subscribe.placeholder": "Tu dirección de correo",
    "subscribe.btn":         "Suscribirse Gratis",
    "subscribe.success":     "✓ ¡Suscrito! Revisa tu bandeja para el correo de bienvenida",
    "subscribe.pending":     "✓ Revisa tu bandeja y haz clic en el enlace de confirmación",
    "subscribe.error":       "Error al suscribirse, inténtalo de nuevo",
    "subscribe.rate":        "Demasiados intentos — inténtalo más tarde",
    "subscribe.invalid":     "Ingresa una dirección de correo válida",
    "subscribe.privacy":     "Respetamos tu privacidad. Sin spam, nunca.",
    "cookie.message":        "Usamos cookies y herramientas de analisis para mejorar tu experiencia.",
    "cookie.accept":         "Aceptar",
    "cookie.decline":        "Solo esenciales",
    "cookie.learnMore":      "Politica de privacidad",
    "nav.tools":             "Herramientas",
    "nav.tools.fee":         "Calculadora",
    "nav.tools.portfolio":   "Portafolio",
    "nav.tools.feargreed":   "Miedo y Codicia",
    "nav.tools.gas":         "Gas Tracker",
    "nav.tools.defi":        "DeFi TVL",
    "nav.tools.glossary":    "Glosario",
    "nav.tools.whale":       "Rastreo Ballenas",
    "bnav.home":             "Inicio",
    "bnav.flash":            "Flash",
    "bnav.prices":           "Precios",
    "bnav.watchlist":        "Favoritos",
    "bnav.search":           "Buscar",
    "watchlist.title":       "Mis favoritos",
    "watchlist.empty":       "Toca ☆ para agregar monedas",
    "watchlist.since":       "desde tu última visita",
    "history.title":         "Leído recientemente",
    "history.empty":         "Sin historial de lectura",
    "history.clear":         "Limpiar",
  },
  "pt": {
    "site.name":        "Bitaigen",
    "site.desc":        "Bitaigen — preço do Bitcoin, notícias cripto e tutoriais de exchanges. Guias Binance/OKX, introdução ao DeFi, segurança de carteiras e análise de blockchain para iniciantes.",
    "site.desc.short":  "Bitaigen — preço Bitcoin, notícias cripto e guias de exchange",
    "home.h1":          "Bitaigen — Preço do Bitcoin, Notícias Cripto e Tutoriais de Exchanges",
    "home.title":       "Bitaigen — Preço do Bitcoin, Notícias Cripto e Tutoriais",
    "site.keywords":    "preço Bitcoin,criptomoedas,tutorial Binance,guia OKX,DeFi,carteira cripto,blockchain,Ethereum,iniciantes cripto,BTC",
    "nav.home":         "Início",
    "nav.flash":        "Flash",
    "nav.news":         "Notícias",
    "nav.newsCenter":   "Central de Notícias",
    "nav.prices":       "Mercado",
    "nav.learn":        "Aprender",
    "nav.topics":       "Tópicos",
    "nav.search":       "Pesquisar",
    "nav.about":        "Sobre",
    "section.latest":   "Últimas Notícias",
    "section.all":      "Todos os Artigos",
    "section.more":     "Ver Mais",
    "section.flash":    "Notícias Rápidas",
    "section.hot":      "Em Alta",
    "section.tutorial": "Tutoriais",
    "section.related":  "Artigos Relacionados",
    "section.tags":     "Tags Populares",
    "section.topic":    "Tópicos",
    "section.about":    "Sobre Nós",
    "topbar.prices":    "Preços ao Vivo",
    "topbar.binance":   "Guia Binance",
    "topbar.telegram":  "Telegram",
    "topbar.follow":    "Siga-nos",
    "topbar.search":    "Pesquisar",
    "topbar.subscribe": "Inscrever-se",
    "topbar.login":     "Contato",
    "footer.col.news":  "Notícias Cripto",
    "footer.col.guide": "Guia para Iniciantes",
    "footer.col.links": "Links Rápidos",
    "footer.col.social":"Redes Sociais",
    "footer.about":     "Sobre Nós",
    "footer.rights":    "Todos os direitos reservados",
    "footer.slogan":    "Inteligência Profissional de Blockchain",
    "footer.disclaimer":"Aviso: O conteúdo deste site é apenas informativo e não constitui conselho de investimento.",
    "post.prev":        "Anterior",
    "post.next":        "Próximo",
    "post.toc":         "Índice",
    "post.author":      "Autor",
    "post.translations":"Outros idiomas",
    "post.readtime":    "Tempo de leitura",
    "post.share":       "Compartilhar",
    "pagination.prev":  "Anterior",
    "pagination.next":  "Próximo",
    "search.title":     "Pesquisar",
    "search.subtitle":  "Pesquisar artigos, tutoriais, análise de mercado…",
    "search.hot":       "Pesquisas Populares",
    "search.quicknav":  "Navegação Rápida",
    "search.btc":       "Preço BTC",
    "search.eth":       "Preço ETH",
    "search.exchange":  "Guias de Exchange",
    "search.beginner":  "Guia Iniciante",
    "search.wallet":    "Segurança Carteira",
    "search.defi":      "Notícias DeFi",
    "archives.title":   "Arquivo Ghost",
    "archives.articles":"artigos",
    "archives.unavailable":"Conteúdo temporariamente indisponível",
    "archives.unavailableDesc":"O serviço Ghost CMS está temporariamente indisponível. Tente novamente mais tarde.",
    "archives.empty":   "Nenhum artigo encontrado",
    "archives.local":   "Artigos locais",
    "about.btc.title":  "Notícias Bitcoin",
    "about.btc.desc":   "Preço do Bitcoin em tempo real, análise de mercado e relatórios detalhados.",
    "about.ex.title":   "Guias de Exchange",
    "about.ex.desc":    "Tutoriais completos da Binance, OKX e outras exchanges principais.",
    "about.wallet.title":"Segurança de Carteira",
    "about.wallet.desc":"Guias de MetaMask, Ledger e outras carteiras para proteger seus ativos.",
    "btn.submit":       "Enviar Notícia",
    "btn.partner":      "Seja Parceiro",
    "btn.telegram":     "Entrar no Telegram",
    "lang.label":       "Idioma",
    "lang.auto":        "Detecção automática",
    "cat.btc":          "BTC",
    "cat.eth":          "ETH",
    "cat.market":       "Mercado",
    "cat.beginner":     "Iniciante",
    "cat.news":         "Notícias",
    "cat.bitcoin":      "Bitcoin",
    "cat.coinswiki":    "Enciclopédia de Moedas",
    "cat.blockchain":   "Blockchain",
    "cat.basics":       "Básicos de Cripto",
    "cat.exchange":     "Exchange",
    "cat.wallet":       "Carteira",
    "cat.defi":         "DeFi",
    "cat.nft":          "NFT",
    "cat.tutorial":     "Tutorial",
    // ── Footer links ──
    "footer.link.btc":       "BTC Notícias",
    "footer.link.eth":       "ETH Notícias",
    "footer.link.defi":      "DeFi Atualizações",
    "footer.link.nft":       "NFT Notícias",
    "footer.link.market":    "Análise de Mercado",
    "footer.link.beginner":  "Guia Iniciante",
    "footer.link.exchange":  "Tutoriais Exchange",
    "footer.link.wallet":    "Segurança Carteira",
    "footer.link.prices":    "Preços ao Vivo",
    "footer.link.btc_price": "Preço BTC",
    "footer.link.all":       "Todos os Artigos",
    "footer.link.tags":      "Índice de Tags",
    "footer.link.archives":  "Arquivo",
    // ── Subscribe ──
    "subscribe.title":       "Assinar Bitaigen",
    "subscribe.desc":        "Notícias cripto semanais e análise de preço do Bitcoin direto no seu e-mail",
    "subscribe.placeholder": "Seu endereço de e-mail",
    "subscribe.btn":         "Assinar Grátis",
    "subscribe.success":     "✓ Inscrito! Verifique seu e-mail para a mensagem de boas-vindas",
    "subscribe.pending":     "✓ Verifique seu e-mail e clique no link de confirmação",
    "subscribe.error":       "Falha na inscrição, tente novamente",
    "subscribe.rate":        "Muitas tentativas — tente novamente mais tarde",
    "subscribe.invalid":     "Insira um endereço de e-mail válido",
    "subscribe.privacy":     "Respeitamos sua privacidade. Sem spam, jamais.",
    "cookie.message":        "Usamos cookies e ferramentas de analise para melhorar sua experiencia.",
    "cookie.accept":         "Aceitar",
    "cookie.decline":        "Apenas essenciais",
    "cookie.learnMore":      "Politica de privacidade",
    "nav.tools":             "Ferramentas",
    "nav.tools.fee":         "Calculadora",
    "nav.tools.portfolio":   "Portfólio",
    "nav.tools.feargreed":   "Medo e Ganância",
    "nav.tools.gas":         "Gas Tracker",
    "nav.tools.defi":        "DeFi TVL",
    "nav.tools.glossary":    "Glossário",
    "nav.tools.whale":       "Rastreio Baleias",
    "bnav.home":             "Início",
    "bnav.flash":            "Flash",
    "bnav.prices":           "Preços",
    "bnav.watchlist":        "Favoritos",
    "bnav.search":           "Buscar",
    "watchlist.title":       "Meus favoritos",
    "watchlist.empty":       "Toque ☆ para adicionar moedas",
    "watchlist.since":       "desde sua última visita",
    "history.title":         "Lidos recentemente",
    "history.empty":         "Sem histórico de leitura",
    "history.clear":         "Limpar",
  },
};

// ── 工具函数 ──────────────────────────────────────────────

export function getLang(url: URL): Lang {
  const firstSeg = url.pathname.split("/").filter(Boolean)[0] ?? "";
  const lower = firstSeg.toLowerCase();
  if (lower in LANG_PATH_MAP) return LANG_PATH_MAP[lower];
  const hl = url.searchParams.get("hl");
  if (hl && hl in LANGUAGES) return hl as Lang;
  return DEFAULT_LANG;
}

export function stripLangPrefix(pathname: string): string {
  for (const seg of LANG_PATH_SEGMENTS) {
    if (pathname === `/${seg}` || pathname.startsWith(`/${seg}/`)) {
      return pathname.slice(seg.length + 1) || "/";
    }
  }
  return pathname;
}

export function hreflangLinks(url: URL): { lang: string; href: string }[] {
  const origin = url.origin;
  const cleanPath = stripLangPrefix(url.pathname);
  return (Object.keys(LANGUAGES) as Lang[]).map(l => {
    const pathSeg = LANG_TO_PATH[l];
    const href = pathSeg
      ? `${origin}/${pathSeg}${cleanPath === "/" ? "" : cleanPath}`
      : `${origin}${cleanPath}`;
    // Use HTML_LANG for hreflang attribute (e.g. pt → pt-BR)
    return { lang: HTML_LANG[l], href };
  });
}

export const buildAlternates = hreflangLinks;

export function useTranslations(lang: Lang) {
  return function t(key: string): string {
    return UI[lang]?.[key] ?? UI[DEFAULT_LANG]?.[key] ?? key;
  };
}
